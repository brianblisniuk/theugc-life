...
export function parseArgs(argv: readonly string[]): Args {
  ...
  if (environment !== "evaluation")
    throw new Error(
      "--environment evaluation is required; production provider ingestion does not exist.",
    );
...
async function main(): Promise<void> {
  ...
  try {
    ...
    for (const row of bundleRows.rows) {
      ...
      const canonical = related.find(
        (c) => c.kind === "canonical_hotel" && c.status === "accepted",
      );
      ...
      console.info(JSON.stringify(evaluatePreview(input, args.asOf), null, 2));
    }
  } finally {
    await client.end();
  }
